...
int main() {
    ...
    // User1 swipes right on User2 (creating a match)
    std::cout << "User1 swipes right on User2" << std::endl;
    app->swipe("user1", "user2", SwipeAction::RIGHT);
    
    // Send messages in the chat room
    std::cout << "\n---- Chat Room ----" << std::endl;
    app->sendMessage("user2", "user1", "Hi Naveen, Kaise ho?");
    
    app->sendMessage("user1", "user2", "Hi Neha, Ma bdiya tum btao");
    
    // Display the chat room
    app->displayChatRoom("user2", "user1");
